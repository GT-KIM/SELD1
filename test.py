...
def test_epoch2(data_generator, model, criterion, dcase_output_folder, params, device):
    # Number of frames for a 60 second audio with 100ms hop length = 600 frames
    # Number of frames in one batch (batch_size* sequence_length) consists of all the 600 frames above with zero padding in the remaining frames
    test_filelist = data_generator.get_filelist()
    ...
    file_cnt = 0
    with torch.no_grad():
        for data_all, target_all in data_generator.generate():
            # load one batch of data
            idx = 0
            outputs = [list() for _ in range(target_all.shape[0] + 49)]
            while idx < data_all.shape[0] :
                data, target = data_all[idx : (idx+72)], target_all[idx : (idx+72)]
                data, target = torch.tensor(data).to(device).float(), torch.tensor(target).to(device).float()
                ...
                for i in range(output.shape[0]) :
                    for j in range(50) :
                        outputs[idx+i+j].append(output[i, j])
                idx += 72
            output = list()
            for curr_output in outputs :
                output.append(np.mean(np.array(curr_output), axis=0))
            output = np.array(output)[np.newaxis, ...]

            if params['multi_accdoa'] is True:
                sed_pred0, doa_pred0, sed_pred1, doa_pred1, sed_pred2, doa_pred2 = get_multi_accdoa_labels(
                    output, params['unique_classes'])
                sed_pred0 = reshape_3Dto2D(sed_pred0)
                doa_pred0 = reshape_3Dto2D(doa_pred0)
                sed_pred1 = reshape_3Dto2D(sed_pred1)
                doa_pred1 = reshape_3Dto2D(doa_pred1)
                ...
                sed_pred, doa_pred = get_accdoa_labels(output, params['unique_classes'])
                sed_pred = reshape_3Dto2D(sed_pred)
                doa_pred = reshape_3Dto2D(doa_pred)
...
def train_epoch(data_generator_A, data_generator_B, optimizer, model, criterion, params, device):
    ...
    for batch1, batch2 in zip(data_generator_A.generate(), data_generator_B.generate()):
        ...
        plt.figure()
        plt.subplot(7, 1, 1)
        specshow(data[0, 0], cmap=cm.jet)
        plt.subplot(7, 1, 2)
        specshow(data[0, 1], cmap=cm.jet)
        plt.subplot(7, 1, 3)
        specshow(data[0, 2], cmap=cm.jet)
        plt.subplot(7, 1, 4)
        specshow(data[0, 3], cmap=cm.jet)
        plt.subplot(7, 1, 5)
        specshow(data[0, 4], cmap=cm.jet)
        plt.subplot(7, 1, 6)
        specshow(data[0, 5], cmap=cm.jet)
        plt.subplot(7, 1, 7)
        specshow(data[0, 6], cmap=cm.jet)
        plt.show()
        plt.clf()
        plt.close()
        """
...
def main():
    ...
    for split_cnt, split in enumerate(test_splits):
        ...
        model_param = model.to('cpu').state_dict()

        params_final = list()
        model_temp = Proposed_model2.Networks(data_out)
        for i in range(1, 10, 1) :
            model_name = '{}_model_backup{}.h5'.format(os.path.join(params['model_dir'], unique_name), i)
            model_temp.load_state_dict(torch.load(model_name, map_location='cpu'))
            params_final.append(model_temp.state_dict())

        dict_params = dict(model_param)
        for name1, param1 in dict_params.items() :
            dict_params[name1].data.copy_(dict_params[name1].data / 10.)

        for i in range(9) :
            for name1, param1 in params_final[i].items() :
                if name1 in dict_params :
                    dict_params[name1].data.copy_(param1.data / 10 + dict_params[name1].data)
        ...
        data_gen_test = cls_data_generator.DataGenerator(
            params=params, split=test_splits[split_cnt], shuffle=False, per_file=True, is_test2=True
        )
        ...
        #test_loss = test_epoch2(data_gen_test, model, criterion, dcase_output_test_folder, params, device)
