...
class FeatureClass:
    def __init__(self, params, is_eval=False):
        ...
        self._datasets = os.listdir(self._dataset_dir)

        self._aud_dirs = [os.path.join(self._dataset_dir, dataset, 'foa') for dataset in self._datasets]
        self._desc_dirs = [os.path.join(self._dataset_dir, dataset, 'metadata') for dataset in self._datasets]
    ...
    def get_frame_stats(self):
        ...
        for aud_dir, desc_dir in zip(self._aud_dirs, self._desc_dirs) :
            print('\t\taud_dir {}\n\t\tdesc_dir {}\n\t\tfeat_dir {}'.format(aud_dir, desc_dir, self._feat_dir))
            for file_cnt, file_name in enumerate(os.listdir(aud_dir)):
                wav_filename = '{}.wav'.format(file_name.split('.')[0])
                with contextlib.closing(wave.open(os.path.join(aud_dir, wav_filename), 'r')) as f:
    ...
    def _get_IPD(self, linear_spectra):
        IPD_feat = np.zeros((linear_spectra.shape[0], linear_spectra.shape[1], 2, linear_spectra.shape[-1] - 1))
        _, ref_channel_phase = librosa.magphase(linear_spectra[:, :, 0])
        ref_channel_phase = np.angle(ref_channel_phase)
        for ch_cnt in range(1, linear_spectra.shape[-1]):
            _, tgt_channel_phase = librosa.magphase(linear_spectra[:, :, ch_cnt])
            tgt_channel_phase = np.angle(tgt_channel_phase)
            IPD_feat[:, :, 0, ch_cnt - 1] = np.sin(ref_channel_phase - tgt_channel_phase)
            IPD_feat[:, :, 1, ch_cnt - 1] = np.cos(ref_channel_phase - tgt_channel_phase)
        IPD_feat = IPD_feat.reshape((linear_spectra.shape[0], linear_spectra.shape[1], 6))  # T, F, C
        IPD_feat = IPD_feat.reshape((linear_spectra.shape[0], linear_spectra.shape[1] * 6))  # T, F * C
    ...
    def extract_file_feature(self, _arg_in,  is_augment=False):
        _file_cnt, _wav_path, _feat_path = _arg_in
        spect = self._get_spectrogram_for_file(_wav_path)
        if is_augment :
            for i in range(8) :
                curr_spect = spect.copy()
                if i == 0:
                    curr_spect[..., 0], curr_spect[..., 1], curr_spect[..., 2], curr_spect[..., 3] = spect[..., 0], -spect[..., 3], -spect[..., 2], spect[..., 1]
                elif i == 1:
                    curr_spect[..., 0], curr_spect[..., 1], curr_spect[..., 2], curr_spect[..., 3] = spect[..., 0], -spect[..., 3], spect[..., 2], -spect[..., 1]
                elif i == 2:
                    curr_spect[..., 0], curr_spect[..., 1], curr_spect[..., 2], curr_spect[..., 3] = spect[..., 0], spect[..., 1], spect[..., 2], spect[..., 3]
                elif i == 3:
                    curr_spect[..., 0], curr_spect[..., 1], curr_spect[..., 2], curr_spect[..., 3] = spect[..., 0], -spect[..., 1], -spect[..., 2], spect[..., 3]
                elif i == 4:
                    curr_spect[..., 0], curr_spect[..., 1], curr_spect[..., 2], curr_spect[..., 3] = spect[..., 0], spect[..., 3], -spect[..., 2], -spect[..., 1]
                    ...
                    curr_spect[..., 0], curr_spect[..., 1], curr_spect[..., 2], curr_spect[..., 3] = spect[..., 0], spect[..., 1], -spect[..., 2], -spect[..., 3]
                mel_spect = self._get_mel_spectrogram(curr_spect)

                # extract intensity vectors
                foa_iv = self._get_foa_intensity_vectors(curr_spect)
                #IPD = self._get_IPD(spect)
                #feat = np.concatenate((mel_spect, foa_iv, IPD), axis=-1)
                ...
                plt.subplot(2, 1, 1)
                specshow(mel_spect, cmap=cm.jet)
                plt.subplot(2, 1, 2)
                specshow(foa_iv, cmap=cm.jet)
                plt.show()
                """

                print('{}: {}, {}'.format(_file_cnt, os.path.basename(_wav_path), feat.shape))
                np.save(_feat_path + '_{}.npy'.format(i), feat)
        else :
    ...
    def extract_all_feature(self):
        ...
        for aud_dir, desc_dir in zip(self._aud_dirs, self._desc_dirs) :
            print('\t\taud_dir {}\n\t\tdesc_dir {}\n\t\tfeat_dir {}'.format(
                aud_dir, desc_dir, self._feat_dir))
            arg_list = []
            for file_cnt, file_name in enumerate(os.listdir(aud_dir)):
                wav_filename = '{}.wav'.format(file_name.split('.')[0])
                wav_path = os.path.join(aud_dir, wav_filename)
                feat_path = os.path.join(self._feat_dir, '{}'.format(wav_filename.split('.')[0]))
                if int(file_name.split('_')[0][4:]) == 1 or int(file_name.split('_')[0][4:]) == 2 or int(
                        file_name.split('_')[0][4:]) == 3 or int(file_name.split('_')[0][4:]) == 4:
                    self.extract_file_feature((file_cnt, wav_path, feat_path), is_augment=True)
                else :
                    self.extract_file_feature((file_cnt, wav_path, feat_path), is_augment=False)
    ...
    def extract_all_labels(self):
        ...
        for aud_dir, desc_dir in zip(self._aud_dirs, self._desc_dirs) :
            print('\t\taud_dir {}\n\t\tdesc_dir {}\n\t\tlabel_dir {}'.format(aud_dir, desc_dir, self._label_dir))
            create_folder(self._label_dir)
            for file_cnt, file_name in enumerate(os.listdir(desc_dir)):
                wav_filename = '{}.wav'.format(file_name.split('.')[0])
                nb_label_frames = self._filewise_frames[file_name.split('.')[0]][1]
                desc_file_polar = self.load_output_format_file(os.path.join(desc_dir, file_name))
                if int(file_name.split('_')[0][4:]) == 1 or int(file_name.split('_')[0][4:]) == 2 or int(
                        file_name.split('_')[0][4:]) == 3 or int(file_name.split('_')[0][4:]) == 4 :
                    for i in range(8) :
                        desc_file = self.convert_output_format_polar_to_cartesian(desc_file_polar, i, is_augment=True)
                        ...
                        np.save(os.path.join(self._label_dir, '{}_{}.npy'.format(wav_filename.split('.')[0], i)), label_mat)
                else :
                    desc_file = self.convert_output_format_polar_to_cartesian(desc_file_polar, None, is_augment=False)
                    if self._multi_accdoa:
                        label_mat = self.get_adpit_labels_for_file(desc_file, nb_label_frames)
                    else:
                        label_mat = self.get_labels_for_file(desc_file, nb_label_frames)
    ...
    def convert_output_format_polar_to_cartesian(self, in_dict, i, is_augment =False):
        out_dict = {}
        for frame_cnt in in_dict.keys():
            if frame_cnt not in out_dict:
                ...
                for tmp_val in in_dict[frame_cnt]:
                    ...
                    if is_augment :
                        if i == 0 :
                            azi_rad, ele_rad = azi_rad - np.pi/2, -ele_rad
                        elif i == 1 :
                            azi_rad, ele_rad = -azi_rad - np.pi/2, ele_rad
                        elif i == 2 :
                            azi_rad, ele_rad = azi_rad, ele_rad
                        elif i == 3 :
                            azi_rad, ele_rad = -azi_rad, -ele_rad
                        elif i == 4 :
                            azi_rad, ele_rad = azi_rad + np.pi/2, -ele_rad
                            ...
                            azi_rad, ele_rad = azi_rad + np.pi, ele_rad
                        elif i == 7 :
                            azi_rad, ele_rad = -azi_rad + np.pi, -ele_rad
                    if azi_rad > np.pi :
                        azi_rad -= 2 * np.pi
    ...
    def convert_output_format_cartesian_to_polar(self, in_dict, i, is_augment=False):
        out_dict = {}
        for frame_cnt in in_dict.keys():
            if frame_cnt not in out_dict:
                out_dict[frame_cnt] = []
                for tmp_val in in_dict[frame_cnt]:
                    ...
                    if is_augment :
                        if i == 0 :
                            azimuth, elevation = azimuth + np.pi/2, -elevation
                        elif i == 1 :
                            azimuth, elevation = -(azimuth + np.pi/2), elevation
                        elif i == 2 :
                            ...
                            azimuth, elevation = azimuth - np.pi, elevation
                        elif i == 7 :
                            azimuth, elevation = -(azimuth - np.pi), -elevation
    ...
    def get_normalized_feat_dir(self):
        return os.path.join(self._feat_label_dir, 'foa_norm')

    def get_unnormalized_feat_dir(self):
        return os.path.join(self._feat_label_dir, 'foa')

    def get_label_dir(self):
        return os.path.join(self._feat_label_dir,'{}_label'.format('foa_adpit' if self._multi_accdoa else 'foa'))

    def get_normalized_wts_file(self):
        return os.path.join(self._feat_label_dir,'foa_wts')
